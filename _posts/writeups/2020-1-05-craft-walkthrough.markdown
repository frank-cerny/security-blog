...
GOGs is “a painless self-hosted git service” per its [homepage](https://gogs.io/){:target="_blank"} ([3]). Think of it as a private GitHub. Instead of hosting repos on github.com, they can be hosted on a private instance of GOGs. Then they can be cloned to a local machine and used nearly the same way as you would for a repository hosted on GitHub.
...
(/bin/bash was not on the box) to pop another reverse shell to my local machine. Thankfully this one did not echo every HTTP request to the box. We also upgraded that “dumb” shell to a TTY shell by following [Rop Nop TTY Tutorial]{:target="_blank"} ([4]).
